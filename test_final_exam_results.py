#!/usr/bin/env python3
"""
Test final exam results functionality
"""
...
def test_final_exam_results():
    """Test final exam results functionality"""
    supabase: Client = create_client(SUPABASE_URL, SUPABASE_SERVICE_KEY)
    
    print("🧪 Testing final exam results functionality...")
    ...
    try:
        ...
        test_data = [
            # Overall score
            {
                ...
                'test_type': 'practice',  # Use 'practice' as test_type
                'category': 'ALL',        # Use 'ALL' as category for overall score
                'test_number': 1,
                'score': 85.0,
                'exam_type': 'CS'
            },
        ...
        print("📝 Inserting test exam results...")
        ...
        if response.data:
            ...
            overall_response = supabase.table('test_results').select('*').eq('user_id', test_user_id).eq('test_type', 'practice').eq('test_number', 1).eq('category', 'ALL').execute()
            ...
            exam_type_response = supabase.table('test_results').select('*').eq('user_id', test_user_id).eq('test_type', 'practice').eq('exam_type', 'CS').eq('category', 'ALL').execute()
