#!/usr/bin/env python3
"""
Test final examen blanc functionality with proper values
"""
...
def test_final_examen_blanc():
    """Test final examen blanc functionality"""
    supabase: Client = create_client(SUPABASE_URL, SUPABASE_SERVICE_KEY)
    
    print("🧪 Testing final examen blanc functionality...")
    
    # Use one of the existing user IDs
    test_user_id = "171029d7-df14-4721-abb9-fbfbf38f61ce"  # Dj Yepidan (CS exam type)
    
    try:
        # Test data for examen blanc #1 using the correct format
        test_data = [
            # Overall score
            {
                ...
                'category': 'OVERALL',
                'test_number': 1,
                'score': 2.0,  # Use the actual score you got
                'exam_type': 'CS'
            },
            # Subject scores
            {
        ...
        if response.data:
            ...
            for i, record in enumerate(response.data):
                print(f"  {i+1}. {record['category']}: {record['score']}% (test_type: {record['test_type']})")
            
            # Now test retrieving the overall score
            print("\n🔍 Testing retrieval of overall score...")
            overall_response = supabase.table('test_results').select('*').eq('user_id', test_user_id).eq('test_type', 'examen_blanc').eq('test_number', 1).eq('category', 'OVERALL').execute()
            ...
            exam_type_response = supabase.table('test_results').select('*').eq('user_id', test_user_id).eq('test_type', 'examen_blanc').eq('exam_type', 'CS').eq('category', 'OVERALL').execute()
            ...
            print("Now check your exam page - you should see the correct score (2%) for Examen Blanc #1")
