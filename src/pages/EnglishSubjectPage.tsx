...
export default function EnglishSubjectPage() {
  ...
  useEffect(() => {
    const fetchStatistics = async () => {
      ...
      try {
        // Get average score for ANG category from user_attempts table
        const score = await UserAttemptService.getAverageScore(user.id, 'ANG', 'Practice');
        
        // Get test count for ANG category from user_attempts table
        const attempts = await UserAttemptService.getUserAttemptsByCategory(user.id, 'ANG');
  ...
  const resumePractice = () => {
    setView('quiz');
    // Scroll to top when resuming practice
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };
