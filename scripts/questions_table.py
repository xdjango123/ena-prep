...
# Map internal categories to the actual test_type values stored in questions_final
# (questions_final.test_type uses: 'exam_blanc', 'practice', 'free_quiz', 'quick_quiz')
DB_TEST_TYPE_MAP = {
    'exam': 'exam_blanc',
    'practice': 'practice',
    'free_quiz': 'free_quiz',
...
class QuestionIngestionEngine:
    ...
    def _load_existing_final_questions(self, cli_test_type: str, exam_type: str) -> List[Dict[str, Any]]:
        ...
        if not db_test_type:
            logger.warning(
                f"Unsupported test_type for questions_final lookup: {cli_test_type} "
            ...
            for q in batch:
                ...
                try:
                    ...
                    if result.passed:
                        ...
                        # For output, use the normalized test_type (e.g. "exam", "practice")
                        q['test_type'] = internal_test_type
                        
                        filled_questions.append(q)
                        logger.info(
                            f"      ✓ Accepted question "
                            f"({q.get('subject', 'CG')} | {q.get('source_table', '?')}:{q.get('source_id', '?')}) "
                            f"-> test {slot_test_number}, question {slot_question_number} "
                            f"({len(filled_questions)}/{total_needed} slots filled)"
                        )
    ...
    async def run_async(self, test_type: str, exam_type: str, dry_run: bool = False):
        ...
        if internal_test_type not in ['exam', 'practice', 'free_quiz', 'quick_quiz']:
            logger.error(
                f"Unsupported test_type: {test_type} "
                f"(normalized to {internal_test_type}). "
                f"Supported: exam, practice/practice_test, free_quiz, quick_quiz."
            )
            return
        ...
        if internal_test_type == 'exam':
            ...
            # free_quiz / quick_quiz: no strict limits, no test_number/question_number
            logger.info("Configuration: No limits (free_quiz/quick_quiz) – no test_number/question_number")
        ...
        if is_strict_mode:
            ...
            logger.info(f"#Processing {exam_type} / {test_type} (all passing questions from questions_v2)#")
        logger.info(f"{'=' * 60}")
        
        # ---------------------------------------------------------------------
        # STRICT GAP-FILL MODE: exam / practice
        # ---------------------------------------------------------------------
        if is_strict_mode:
        ...
        # ---------------------------------------------------------------------
        # NON-STRICT MODE: free_quiz / quick_quiz
        # ---------------------------------------------------------------------
        else:
            # For free_quiz and quick_quiz, import all passing questions from questions_v2
            # No test_number/question_number assignment, no max per subject.
            logger.info(f"\n#pulling existing questions from questions_final table for duplicate checks...#")
            existing_rows = self._load_existing_final_questions(test_type, exam_type)
            self._preload_validator_with_existing(existing_rows)
            
            logger.info(f"\n#pulling questions from questions_v2 table...#")
            v2_test_types = CATEGORY_TO_V2_TEST_TYPES.get(internal_test_type, [internal_test_type])
            v2_rows = DataFetcher.fetch_from_questions_v2(v2_test_types, exam_type)
            logger.info(f"-> Fetched {len(v2_rows)} questions from questions_v2 table")
            ...
            if not converted_questions:
                logger.info("-> No questions available in questions_v2 for this test_type/exam_type.")
                return
            
            filename_key = f"{exam_type.lower()}_{internal_test_type}_v2_import"
            logger.info(f"\n##question verification (free_quiz/quick_quiz import)##")
            logger.info("-> No strict limits. All questions that pass the 10 checkpoints will be exported.")
                    ...
                    try:
                        ...
                        if result.passed:
                            ...
                            logger.info(
                                f"      ✓ Accepted question "
                                f"({q.get('subject', 'CG')} | {q.get('source_table', '?')}:{q.get('source_id', '?')}) "
                                f"({len(final_questions)} total accepted)"
                            )
                            self.validator.add_to_ingested(q, filename_key)
                        else:
                            failed_count += 1
