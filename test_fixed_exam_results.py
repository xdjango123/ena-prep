#!/usr/bin/env python3
"""
Test fixed exam results functionality
"""
...
def test_fixed_exam_results():
    """Test fixed exam results functionality"""
    supabase: Client = create_client(SUPABASE_URL, SUPABASE_SERVICE_KEY)
    
    print("🧪 Testing fixed exam results functionality...")
    ...
    try:
        ...
        test_data = [
            # Overall score
            {
                ...
                'category': 'OVERALL',    # Use 'OVERALL' as category
                'test_number': 1,
                'score': 85.0,
                'exam_type': 'CS'
            },
        ...
        if response.data:
            ...
            overall_response = supabase.table('test_results').select('*').eq('user_id', test_user_id).eq('test_type', 'practice').eq('test_number', 1).eq('category', 'OVERALL').execute()
            ...
            exam_type_response = supabase.table('test_results').select('*').eq('user_id', test_user_id).eq('test_type', 'practice').eq('exam_type', 'CS').eq('category', 'OVERALL').execute()
